...
const STARTER_CATEGORIES = ['Rent', 'Electricity', 'Gas', 'Water', 'Repairs', 'Transport', 'License / tax', 'Miscellaneous']

export function ExpenseCategoriesSheet({ onClose }: { onClose: () => void }) {
  ...
  function load() {
    Promise.all([ExpenseCategoryApi.list(), OtherExpenseApi.list()])
      .then(([cats, expenses]) => {
        setCategories(sortCategories(cats))
        const counts = new Map<string, number>()
        for (const expense of expenses) {
          if (expense.category) counts.set(expense.category.id, (counts.get(expense.category.id) ?? 0) + 1)
        }
        setItemCounts(counts)
      })
      .catch((err) => setError(err instanceof ApiError ? err.message : 'Could not load expense categories'))
  }
  ...
  async function handleAdd(e: FormEvent) {
    ...
    try {
      ...
      const created = await ExpenseCategoryApi.create(name.trim(), nextOrder)
  ...
  async function seedStarters() {
    ...
    setError(null)
    try {
      const created = await Promise.all(STARTER_CATEGORIES.map((n, i) => ExpenseCategoryApi.create(n, i)))
  ...
  async function reorder(newList: CategoryItem[]) {
    ...
    try {
      await Promise.all(changed.map((c) => ExpenseCategoryApi.update(c.id, { sortOrder: c.sortOrder })))
    } catch (err) {
    ...
    <div className="fixed inset-0 z-30 flex items-end justify-center bg-black/40" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-neutral-100 p-5 pb-3">
          <h2 className="text-base font-bold text-neutral-900">Expense categories</h2>
        ...
        <div className="flex-1 overflow-y-auto px-5">
          ...
          {categories && categories.length > 1 && (
            <p className="mb-1 mt-3 text-xs text-neutral-400">Use the arrows to reorder.</p>
          )}
          {categories?.length === 0 && (
            <div className="py-6 text-center">
              <p className="mb-3 text-sm text-neutral-400">No expense categories yet.</p>
              <button
            ...
            <div key={cat.id} className="flex items-center gap-1 border-b border-neutral-50 py-1.5">
              ...
              >
                ...
                <span className="text-xs text-neutral-400">
                  {itemCounts.get(cat.id) ?? 0} expense{(itemCounts.get(cat.id) ?? 0) === 1 ? '' : 's'}
                </span>
              </button>
            </div>
          ))}
...
function EditExpenseCategorySheet({
  category,
  itemCount,
  onClose,
  onSaved,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    setBusy(true)
    try {
      const updated = await ExpenseCategoryApi.update(category.id, { name: name.trim() })
  ...
  async function handleDelete() {
    ...
    setBusy(true)
    try {
      await ExpenseCategoryApi.remove(category.id)
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="flex gap-2">
            <button
              ...
              title={itemCount > 0 ? `${itemCount} expense(s) still use this category` : undefined}
          ...
          {itemCount > 0 && (
            <p className="text-center text-xs text-neutral-400">
              {itemCount} expense{itemCount === 1 ? '' : 's'} use this category — edit or delete them first to delete
              it.
            </p>
          )}
        </form>
